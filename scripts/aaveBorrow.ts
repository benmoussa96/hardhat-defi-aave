 ...
 */
const getWeth = async (wrappedEtherAddress: string, amount: BigNumber, account: string) => {
...
const getAvailableDaiToBorrow = async (
  ...
  const daiAmountToBorrow = availableBorrowsETH.toString() * 0.95 * (1 / daiEthPrice.toString());
...
const borrow = async (
  ...
  const borrowTxn = await lendingPool.borrow(daiAddress, daiAmountToBorrow, 2, 0, account);
